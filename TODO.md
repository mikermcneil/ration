...
```js
profileImageUploadFd: {
  type: 'string',
  description: 'The Skipper file descriptor string uniquely identifying the uploaded profile image (or empty string if there is no profile image).'
},

profileImageUploadMime: {
  type: 'string',
  description: 'The MIME type for the uploaded profile image (or empty string if there is no profile image).'
},
```
...
### wherever we need to do the upload
...
```js
// …
// Now see if a file was uploaded or not, and react accordingly.
var uploadedFile = await sails.uploadOne(inputs.photo);
if (uploadedFile) {
  Object.assign(valuesToSet, {
    profileImageUploadFd: uploadedFile.fd,
    profileImageUploadMime: uploadedFile.type
  });
}
```
...
module.exports = {
  ...
  friendlyName: 'Download profile image',
  ...
  exits: {
    success: {
      outputDescription: 'The streaming bytes of the specified user\'s profile image.',
      outputType: 'ref'
    },
    redirect: { responseType: 'redirect', description: 'Redirecting to generic default image...' },
  ...
  fn: async function (inputs, exits) {
    ...
    // Check permissions.
    // (so that people can't update each other's profile images)
    if (this.req.me.id !== inputs.id) {
      throw 'forbidden';
    }

    // If there is no corresponding custom image uploaded, then redirect
    // to the URL of the appropriate default image.
    if (!user.profileImageUploadFd) {
      throw {redirect: '/images/default-profile-images/user.png'};
    }

    if (user.profileImageUploadMime) {
      this.res.type(user.profileImageUploadMime);
    }//ﬁ

    var downloading = await sails.startDownload(user.profileImageUploadFd);
...
### api/hooks/uploads/private/verify-upstream.js
